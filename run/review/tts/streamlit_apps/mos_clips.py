...
Usage:
    $ PYTHONPATH=. streamlit run run/review/tts/streamlit_apps/mos_clips.py --runner.magicEnabled=false
"""
import pathlib
import random
import typing
...
def main():
    ...
    with st.spinner("Loading and exporting model(s)..."):
        ...
        packages = [load_tts(c, device=device) for c in checkpoints_keys]
    ...
    for package, checkpoints_ in zip(packages, checkpoints_keys):
        ...
        with st.spinner(f"Generating clips with `{checkpoints_}` checkpoints..."):
            # NOTE: v11 trains on data generated by `make_training_batches` which includes
            # automatically generated annotation and context for the training data, so the model
            # performance will be different on these.
            batches = make_batches([(XMLType(s.script), s.session) for s in spans])
