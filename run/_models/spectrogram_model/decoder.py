...
class Decoder(torch.nn.Module):
    ...
    def __init__(
        ...
        input_size = seq_embed_size + encoder_out_size
    ...
    def _make_hidden_state(self, encoded: Encoded) -> DecoderHiddenState:
        ...
        # [batch_size, seq_embed_size + encoder_out_size] →
    ...
    def forward(
        ...
        **kwargs,
        ...
        for frame in pre_net_frames.split(1, dim=0):
            ...
            # [batch_size, encoder_out_size] →
            # [batch_size, pre_net_hidden_size + encoder_out_size + seq_embed_size]
            frame = torch.cat([frame, last_attention_context, encoded.seq_embed], dim=1)

            # frame [batch (batch_size),
            # input_size (pre_net_hidden_size + encoder_out_size + seq_embed_size)]  →
        ...
        # input_size (lstm_hidden_size + encoder_out_size + seq_embed_size)] →
        # [num_frames, batch_size, lstm_hidden_size]
