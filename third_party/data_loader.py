# NOTE: Patched the `DataLoader` until this issue is resolved:
# https://github.com/pytorch/pytorch/pull/23744#issuecomment-544764001
...
class _DatasetKind(object):
    ...
    def create_fetcher(kind, dataset, auto_collation, collate_fn, drop_last):
        if kind == _DatasetKind.Map:
            ...
            return _utils.fetch._IterableDatasetFetcher(dataset, auto_collation, collate_fn,
                                                        drop_last)
...
class DataLoader(object):
                    ...
                    if multiprocessing_context not in valid_start_methods:
                        raise ValueError(
                            ('multiprocessing_context option '
                             ...
                             'multiprocessing_context={}').format(valid_start_methods,
    ...
    def __setattr__(self, attr, val):
        if self.__initialized and attr in ('batch_size', 'sampler', 'drop_last', 'dataset'):
...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        self._dataset_fetcher = _DatasetKind.create_fetcher(self._dataset_kind, self._dataset,
                                                            self._auto_collation, self._collate_fn,
                                                            self._drop_last)
    ...
    def __init__(self, loader):
        ...
        for i in range(self._num_workers):
            ...
            w = multiprocessing_context.Process(
                ...
                args=(self._dataset_kind, self._dataset, index_queue, self._worker_result_queue,
                      self._workers_done_event, self._auto_collation, self._collate_fn,
                      self._drop_last, self._base_seed + i, self._worker_init_fn, i,
                      self._num_workers))
            w.daemon = True
        ...
        if self._pin_memory:
            ...
            pin_memory_thread = threading.Thread(
                ...
                args=(self._worker_result_queue, self._data_queue, torch.cuda.current_device(),
