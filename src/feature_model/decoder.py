...
class DecoderHiddenState(object):
    ...
    def __init__(self,
        ...
        Args:
            last_attention_context (torch.FloatTensor [batch_size, attention_context_size],
                optional):we condition the decoder on the last attention
                context.
            last_attention_alignment (torch.FloatTensor [num_tokens, batch_size], optional): For
                sequential prediction, we condition the decoder on the last attention alignment.
            last_frame (torch.FloatTensor [1, batch_size, frame_channels], optional): For sequential
                prediction, we condition the decoder on the last frame.
            lstm_one_hidden_state (tuple): The hidden state of the first LSTM in Tacotron.
            lstm_two_hidden_state (tuple): The hidden state of the second LSTM in Tacotron.
        """
...
class Decoder(nn.Module):
    ...
    def _get_past_frames(self, batch_size, ground_truth_frames=None, hidden_state=None):
        ...
        Args:
            ...
            hidden_state (DecoderHiddenState): For sequential prediction, decoder hidden state used
