...
# COPIED FROM:
# https://github.com/pytorch/pytorch/pull/4838


class ZoneoutInplaceFunction(InplaceFunction):
    ...
    def forward(cls,
        ...
        if type(current_input) != type(previous_input):
            raise ValueError('Current and previous inputs must be of the same '
                             ...
                             '{previous}'.format(
                                 current=type(current_input), previous=type(previous_input)))
    ...
    def backward(ctx, grad_output):
        if ctx.train:
            return (grad_output * Variable(ctx.current_mask),
                    grad_output * Variable(ctx.previous_mask), None, None, None, None)
        else:
            return grad_output, None, None, None, None, None


def zoneout(previous_input, current_input, p=None, mask=None, training=False, inplace=False):
    return ZoneoutInplaceFunction.apply(previous_input, current_input, p, mask, training, inplace)
...
class Zoneout(torch.nn.Module):
    ...
    def forward(self, previous_input, current_input):
        return zoneout(previous_input, current_input, self.p, self.mask, self.training,
                       self.inplace)
