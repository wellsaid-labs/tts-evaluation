...
def main(audio_path):
    ...
    rms_level = root_mean_square_from_signal(audio.numpy())
    ...
    weighted_frame_rms_level = framed_root_mean_square_from_power_spectrogram(
        db_to_power(db_mel_spectrogram))
    ...
    plot_mel_spectrogram(
        db_mel_spectrogram, sample_rate=metadata.sample_rate).savefig('db_mel_spectrogram.png')
    logger.info('Wrote `db_mel_spectrogram.png`.')
    plot_spectrogram(db_spectrogram, sample_rate=metadata.sample_rate).savefig('db_spectrogram.png')
    logger.info('Wrote `db_spectrogram.png`.')
    plot_spectrogram(spectrogram, sample_rate=metadata.sample_rate).savefig('spectrogram.png')
    logger.info('Wrote `spectrogram.png`.')
    waveform = griffin_lim(
        db_mel_spectrogram.numpy(), sample_rate=metadata.sample_rate, use_tqdm=True)
    write_audio('griffin_lim.wav', waveform, sample_rate=metadata.sample_rate, overwrite=True)
    logger.info('Wrote `griffin_lim.wav`.')
