...
def _make_args():
    ...
    dataset, (input_encoder, spec_model, sig_model) = _utils.make_mock_tts_bundle()
    ...
    return spec_model, sig_model, encoded
...
def test_text_to_speech_ffmpeg_generator():
    ...
    with fork_rng(seed=123):
        spec_model, sig_model, encoded = _make_args()
        generator = text_to_speech_ffmpeg_generator(logger, spec_model, sig_model, encoded)
...
def test_text_to_speech_ffmpeg_generator__thread_leak():
    ...
    with fork_rng(seed=123):
        ...
        generator = text_to_speech_ffmpeg_generator(logger, spec_model, sig_model, encoded)
        spec_model.stop_threshold = math.inf
        next(generator)
