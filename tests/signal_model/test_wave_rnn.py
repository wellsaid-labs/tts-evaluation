...
def test_wave_rnn_infer_equals_forward():
    ...
    infer_local_features = torch.randn(local_length, local_features_size) * 0.1
    forward_local_features = infer_local_features.unsqueeze(0)
    ...
    forward_hidden_state = hidden_state.unsqueeze(0)
    ...
    infer_predicted_coarse, infer_predicted_fine, infer_hidden_state = net.infer(
        infer_local_features, hidden_state=infer_hidden_state, argmax=True)

    # [signal_length] → [1, signal_length - 1, 2]
    forward_input_signal = torch.stack((infer_predicted_coarse, infer_predicted_fine), dim=1)[:-1]
    forward_input_signal = forward_input_signal.unsqueeze(0)
    # [1] → [1, 1, 2]
    go_signal = torch.stack((go_coarse, go_fine), dim=1).unsqueeze(1).long()
    # [1, signal_length - 1, 2] → [1, signal_length, 2]
    forward_input_signal = torch.cat((go_signal, forward_input_signal), dim=1)
    forward_target_coarse = infer_predicted_coarse.view(1, -1, 1)

    forward_predicted_coarse, forward_predicted_fine, forward_hidden_state = net.forward(
        forward_local_features,
    ...
    infer_hidden_state = torch.cat((infer_coarse_hidden, infer_fine_hidden), dim=0).unsqueeze(0)
    np.testing.assert_allclose(
        forward_hidden_state.detach().numpy(), infer_hidden_state.detach().numpy(), atol=1e-04)

    forward_predicted_coarse = forward_predicted_coarse.max(dim=2)[1].squeeze(0)
    np.testing.assert_allclose(
        infer_predicted_coarse.detach().numpy(),
        forward_predicted_coarse.detach().numpy(),
        atol=1e-04)

    forward_predicted_fine = forward_predicted_fine.max(dim=2)[1].squeeze(0)
    np.testing.assert_allclose(
