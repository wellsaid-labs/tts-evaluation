...
@mock.patch('src.datasets.m_ailabs.compute_spectrogram')
@mock.patch('urllib.request.urlretrieve')
@pytest.mark.usefixtures('cleanup')
def test_m_ailabs_speech_dataset(mock_urlretrieve, mock_compute_spectrogram, mock_from_path,
                                 mock_is_file):
...
@mock.patch('urllib.request.urlretrieve')
@pytest.mark.usefixtures('cleanup')
def test_m_ailabs_speech_dataset_pickers(mock_urlretrieve, mock_compute_spectrogram,
                                         mock_from_path):
    mock_urlretrieve.side_effect = urlretrieve_side_effect
    mock_compute_spectrogram.side_effect = compute_spectrogram_side_effect
    mock_from_path.return_value = Checkpoint(directory='.', model=lambda x: x, step=0)
