...
def test_strip_func():
    """Test `lib.text.utils.strip_func` handles various white space scenarios."""
    assert lib.text.utils.strip_func("  abc1def   ", lambda c: c.isspace()) == "abc1def"
    assert lib.text.utils.strip_func("  abc1def", lambda c: c.isspace()) == "abc1def"
    assert lib.text.utils.strip_func("abc1def   ", lambda c: c.isspace()) == "abc1def"
    assert lib.text.utils.strip_func("abc1def", lambda c: c.isspace()) == "abc1def"
    assert lib.text.utils.strip_func("", lambda c: c.isspace()) == ""
