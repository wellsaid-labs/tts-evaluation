...
def test__save():
    filename = _save(
        Path('tests/_test_data'), ['tag_1', 'tag_2'], Speaker('First Last', Gender.FEMALE),
        np.array([0, 0.5, 0, -0.5]))
    expected_filename = 'speaker=first_last,tag_1,tag_2.wav'
    assert filename == expected_filename
    assert (Path('tests/_test_data') / expected_filename).exists()


def test__save_obscure():
    filename = _save(
        Path('tests/_test_data'), ['tag_1', 'tag_2'],
...
def test_main(capsys):
    ...
    with capsys.disabled():  # Required for the test to pass (could be a bug with PyTest).
        mocks = get_tts_mocks()
        destination = Path('tests/_test_data/samples')
        num_samples = 2
        ...
        assert (destination / metadata_filename).exists()
        assert len(list(destination.glob('*.log'))) == 2
        assert len(list(destination.glob('*.wav'))) == num_samples * 3
...
def test_main__no_checkpoints(capsys):
    ...
    with capsys.disabled():  # Required for the test to pass (could be a bug with PyTest).
        ...
        assert (destination / metadata_filename).exists()
        assert len(list(destination.glob('*.log'))) == 2
        assert len(list(destination.glob('*.wav'))) == num_samples
...
def test_main__text_only_dataset(capsys):
    ...
    with capsys.disabled():  # Required for the test to pass (could be a bug with PyTest).
        ...
        assert (destination / metadata_filename).exists()
        assert len(list(destination.glob('*.log'))) == 2
        assert len(list(destination.glob('*.wav'))) == num_samples * 2
