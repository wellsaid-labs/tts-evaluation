...
class TestOptimizer(unittest.TestCase):
    ...
    def test_init(self):
        ...
        try:
            Optimizer(torch.optim.Adam(params), max_grad_norm_clip=None)
        except:
    ...
    def test_to(self):
        ...
        optim = Optimizer(adam, max_grad_norm_clip=None)
    ...
    def test_step_max_grad_norm_clip(self, mock_clip_grad_norm):
        params = [torch.nn.Parameter(torch.randn(2, 3, 4))]
        optim = Optimizer(torch.optim.Adam(params), max_grad_norm_clip=5)
        optim.step()
    ...
    def test_ignore_step(self):
        ...
        # Test ``max_grad_norm_ignore``
        did_step = False
        params[0].grad = torch.tensor([100.0])
        adam = torch.optim.Adam(params)
        adam.step = _step
        optim = Optimizer(adam, max_grad_norm_ignore=10)
        assert not did_step
