...
1. Build the container image:
   ...
   docker build -f docker/master/Dockerfile -t gcr.io/${PROJECT_ID}/speech-api:v5.04 .
   docker build -f docker/worker/Dockerfile -t gcr.io/${PROJECT_ID}/speech-api-worker:v6.07 .
   ```

1. Check the worker image size:

   ```bash
   docker images gcr.io/${PROJECT_ID}/speech-api-worker:v6.07
   ```
...
1. Push the build:
   ...
   ```bash
   docker push gcr.io/${PROJECT_ID}/speech-api:v5.04
   docker push gcr.io/${PROJECT_ID}/speech-api-worker:v6.07
   ```
...
1. Test the build:
   ...
   docker run --rm -p 8000:8000 -e "YOUR_SPEECH_API_KEY=123" \
      gcr.io/${PROJECT_ID}/speech-api-worker:v6.07
   ```
   ...
   docker run --rm -p 8000:8000 -e "AUTOSCALE_LOOP=5000 YOUR_SPEECH_API_KEY=123" \
      gcr.io/${PROJECT_ID}/speech-api:v5.04
   ```
...
- For authentication reasons, the build should be pushed with the `gcloud` tool:
  ...
  ```bash
  sudo gcloud docker -- push gcr.io/${PROJECT_ID}/speech-api-worker:v6.07
  ```
